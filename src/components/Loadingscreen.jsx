...
export function LoadingScreen() {
    ...
    gsap.to(progressBar, {
      ...
      onUpdate: () => {
        ...
        const progress = Math.round(
          (gsap.getProperty(progressBar, "width") /
            gsap.getProperty(progressBar.parentNode, "width")) *
            100
        );
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center justify-center bg-background dark:bg-blackbg bg-grid-black/[0.2]  dark:bg-dot-black-500/[0.3]">
      <div className="mt-5 text-4xl md:text-9xl font-extrabold text-black">
        Are you ready?💡{" "}
      </div>
      ...
      <div
        id="loadingText"
        className="mt-5 text-7xl md:text-9xl font-extrabold text-black"
      >
        {loadingPercentage < 4 ? "1%" : loadingPercentage < 6 ? "25%" : "100%"}
      </div>
